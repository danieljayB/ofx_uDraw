...
void print_raw_data(std::ostream& out, uint8_t* data, int len)
{
  ...
  for(int i = 0; i < len; ++i)
  {
      std::cout << std::string("[%d]%02x") % i % int(data[i]);
    if (i != len-1)
      std::cout << " ";
  }
       ...
       [&](uint8_t* data, int size)
       {
         ...
         else if (opts.accelerometer_mode)
         {
           ...
           else
           {
             ...
             {
               std::cout << std::string("finger: x: %4d y: %4d") % x % y;
             }
             else if (data[11] == 0x40)
             {
               std::cout << std::string("pen: x: %4d y: %4d  - pressure: %3d") % x % y % (int(data[13]) - 0x70);
             }
             else
             {
               std::cout << std::string("pinch: x: %4d y: %4d  distance: %4d  orientation: %02x") % x % y % int(data[12]) % (int(data[11]) - 0xc0);
             }
             ...
             // accelerometer
             std::cout << std::string("%4d %4d %4d - %4d %4d %4d - %4d %4d %4d") %
               acc_x % acc_y % acc_z %
